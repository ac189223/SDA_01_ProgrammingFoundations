...
public class Test {
    ...
    private Circle headAppears(Canvas canvas) {
        headOfSnake = new Circle(canvas.getWidth() / 4, canvas.getHeight() / 4, 4, "red");
    ...
    private void setTarget(Canvas canvas) {
        boolean check = false;
        while (!check) {
            targetCircle = new Circle(random.nextInt(canvas.getWidth()), random.nextInt(canvas.getHeight()), 4, "green");
    ...
    private boolean checkDistance(Circle circle1, Circle circle2) {
        if (distance(circle1, circle2) < circle1.getRadius() + circle2.getRadius())
            return false;
        return true;
    }

    private double distance(Circle circle1, Circle circle2) {
        return squareRoot(square(circle1.getXMiddle() - circle2.getXMiddle()) +
                square(circle1.getYMiddle() - circle2.getYMiddle()));
    }
    ...
    private void moveSnakeHorizontal(int step) {
        for (int i = snake.size() - 1; i > 0; i--) {
            snake.get(i).setXMiddle(snake.get(i - 1).getXMiddle());
            snake.get(i).setYMiddle(snake.get(i - 1).getYMiddle());
            headOfSnake.setXMiddle(headOfSnake.getXMiddle() + step);
        }
    }
    ...
    private void moveSnakeVertical(int step) {
        for (int i = snake.size() - 1; i > 0; i--) {
            snake.get(i).setXMiddle(snake.get(i - 1).getXMiddle());
            snake.get(i).setYMiddle(snake.get(i - 1).getYMiddle());
            headOfSnake.setYMiddle(headOfSnake.getYMiddle() + step);
        }
    }
    ...
    private void eatTarget() {
        ...
        for (int i = snake.size() - 1; i > 0; i--) {
            ...
            headOfSnake.setXMiddle(targetCircle.getXMiddle());
            headOfSnake.setYMiddle(targetCircle.getYMiddle());
        }
    }
    ...
    private boolean biteHimself() {
        for (Circle fromSnake: snake)
            if (!checkDistance(fromSnake, headOfSnake))
                return true;
        return false;
    }
